...
@include('file.body')
@livewire('navigation')
<div class="w-full h-full flex flex-col" style="padding-top: 100px">
 
    @foreach ($users as $user)
        <div class="w-90 m-5 rounded-lg bg-zinc flex" style="margin-top:10px">
            <div class="">
            ...
            <div class="">
            <a href="/profile/{{$user->id}}"><h1 class="pl-5 pt-5 font-semibold">{{$user->name}}</h1></a>
            <p class="pl-5"> {{$user->bio}}</p>
             </div>
        </div>
    @endforeach
</div>
