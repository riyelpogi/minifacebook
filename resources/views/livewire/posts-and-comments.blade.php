<div>
    <div class="w-full flex justify-center items-center mt-5 flex-col " >
        
        @foreach ($posts as $post)
            <div class="mt-5 rounded-lg shadow-lg w-11/12 sm:w-3/6" style="background-color:#00B8A9">
                <div class="m-3 shadow-2xl" style="min-height:100px;max-height:500px">
                    <form action="/profile/{{$post->user->id}}" method="GET">
                    @csrf
                        <button type="submit"><h1 class="font-bold pt-1 flex">@if ($post->user->displaypicture != null)
                ...
                <div class="min-w-0 overflow-y-scroll no-scrollbar" style="min-height:100px; max-height:200px">
                   
                    @foreach ($post->comments as $comment)
                        <div class="max-w-fit min-w-0 mb-2 ml-7 mr-2 p-2 rounded-xl shadow-lg bg-white"  >
                            <ul>
                          <li class="mim-w-min flex">
                            ...
                            @endif
                            <a Href="/profile/{{$comment->user->id}}" ><p class="break-words pt-0.5 font-bold pl-2"> {{$comment->user->name }} </p></a></li>
                ...
                <div class="flex justify-center items-center" style="height:80px;gap:10px">
                    <form wire:submit.prevent="storeComment({{$post->id}})" wire:key="post.{{$post->id}}" >
                        @csrf
                        <input type="text" wire:model.defer="comment" wire:key="comment.{{$post->id}}" id="" class="rounded-2xl pl-2 bg-slate shadow-lg w-28 sm:w-40" placeholder="Comment here" >
